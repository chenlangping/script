...
def get_public_ip():
    ...
    try:
        ...
        ip = subprocess.check_output(command).decode().strip()
...
def generate_ssr_link(config):
    ...
    try:
        ...
        password_b64 = base64.urlsafe_b64encode(password.encode('utf-8')).decode().rstrip('=')
        ...
        params = {}
        if obfs_param:
            obfsparam_b64 = base64.urlsafe_b64encode(obfs_param.encode('utf-8')).decode().rstrip('=')
            params['obfsparam'] = obfsparam_b64
        if protocol_param:
            protoparam_b64 = base64.urlsafe_b64encode(protocol_param.encode('utf-8')).decode().rstrip('=')
        ...
        encoded_full_str = base64.urlsafe_b64encode(full_str.encode('utf-8')).decode().rstrip('=')

        # 返回最终的 ssr:// 链接
        return "ssr://" + encoded_full_str

    except Exception as e:
        print("[!] 生成链接时出错: " + str(e), file=sys.stderr)
        return None
...
def main():
    ...
    try:
        ...
        print("[!] 错误: 找不到或无权限读取配置文件 '" + CONFIG_FILE_PATH + "'。", file=sys.stderr)
        print("    请确认文件存在且权限正确，或尝试使用 'sudo' 运行此脚本。", file=sys.stderr)
        sys.exit(1)
    except ValueError:
        print("[!] 错误: 配置文件 '" + CONFIG_FILE_PATH + "' 格式不正确。", file=sys.stderr)
        sys.exit(1)
